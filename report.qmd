...
author: "Hudson, Isabella, Jacob, and Hayat"
format: html
execute:
  echo: true
  warning: false
...
- Team member 2: _Hayat Bounite / Normalized and plotted data set A_
- Team member 3: _Hudson Finnochio / Normalized and plotted data set C_
- Team member 4: _Jacob Alexander / Did Part 2, the McCulloch–Pitts Neuron_
...
# Loading dataset
df = pd.read_csv("data/groupA.txt", sep=",", header=None, names=["Price", "Weight", "Type"])
...
$$
f(x) \ge 0 \quad\Longrightarrow\quad \text{Predict Type} = 1 \;(\text{big car})
$$
...
# table matching your classmate's format (TP, FP / FN, TN)
...
TPR = tp / (tp + fn) if (tp + fn) else float("nan")   # recall / sensitivity (class 1: big)
TNR = tn / (tn + fp) if (tn + fp) else float("nan")   # specificity (class 0: small)
...
_Justification:_ The neuron fires (\(y=1\)) whenever \(X=0\) or \(Y=1\), regardless of the value of \(Z\).
